...
$(document).on('mouseover', '.image_preview', function (e) {
    ...
    if (image != "") {
        var zoomView = $('<img src="' + image + '" id="zoomView" class="zoomView_img"/>'); // 建立图片查看框
        $(this).after(zoomView);
        $("#zoomView").fadeIn(100);
...
$(document).on('mouseover', '.images_preview', function (e) {
    ...
    if (image != "") {
		...
		var image_str = '<div id="zoomView">';
		$.each(image_json,function(index,obj){
            if(typeof obj.small == "undefined" || obj.small == null || obj.small == ""){
                var scr_vlaue=obj.image;
            }else{
        ...
        var zoomView = $(image_str); // 建立图片查看框
        $(this).after(zoomView);
        $("#zoomView").fadeIn(100);
        $("#zoomView").css({"top": (e.pageY - 250) + "px", "left": (e.pageX - 210) + "px"});  //注意得在CSS文件中将其设置为绝对定位
    }
})
$(document).on('mousemove', '.images_preview', function (e) {
    var image = $(this).attr("image");
    if (image != "") {
        $("#zoomView").css({"top": (e.pageY - 250) + "px", "left": (e.pageX - 210) + "px"}); //鼠标移动时及时更新图片查看框的坐标
    }
})
$(document).on('mouseout', '.images_preview', function (e) {
    var image = $(this).attr("image");
    if (image != "") {
        $("#zoomView").remove();    //鼠标移出时删除之前建立的图片查看框
    }
