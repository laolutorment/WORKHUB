...
(function ($) {
    $.extend({
        ...
        table: {
            ...
            init: function (options) {
                ...
                var defaults = {
                    ...
                    pageList: [10, 25,50,100,'all'],  // 12.位置编号ZTX-010，设置列表显示所有记录时，页面调整。
					//原代码：pageList: [10, 25,50,100],  // 12.位置编号ZTX-010，设置列表显示所有记录时，页面调整。
            ...
            queryParams: function (params) {
				//12.位置编号ZTX-010，设置列表显示所有记录时，页面调整。
				if($.btTable.bootstrapTable('getOptions').pageSize==='所有'){
					params.limit=$.btTable.bootstrapTable('getOptions').totalRows;
				}
				//12.位置编号ZTX-010，设置列表显示所有记录时，页面调整。
                var curParams = {
                    // 传递参数查询参数
            ...
            search: function (formId, data) {
                ...
                //ZTX每次重新搜索时，把当前页设置为1，解决在浏览页数大于1时搜索结果不显示的问题
                params.pageNumber = 1;                
        ...
        operate: {
            ...
            // ZTX数据表导出
            database_export: function (id) {
                var id=$.common.isEmpty($.table._option.uniqueId) ? $.table.selectFirstColumns() : $.table.selectColumns($.table._option.uniqueId);
                var url = '/admin/Database/export.html?id='+id;
                if ($.table._option.layerOpen == "1") {
                    // 通过参数隐藏左侧和顶部等数据
                    if (url.indexOf('?') != -1) {
                        url = url + '&_layer=1'
            ...
            // 数据库导入
            database_import: function (id) {
                var id=$.common.isEmpty($.table._option.uniqueId) ? $.table.selectFirstColumns() : $.table.selectColumns($.table._option.uniqueId);
                var url = '/admin/Database/excel_import.html?id='+id;
                if ($.table._option.layerOpen == "1") {
                    // 通过参数隐藏左侧和顶部等数据
                    if (url.indexOf('?') != -1) {
                        url = url + '&_layer=1'
                    ...
                    layer.open({
                        ...
                        yes:function (index, layero) {
                            ...
                            //iframeWin.contentWindow.submitHandler(index, layero);
                            // 获取弹出层中的form表单元素
                           var formSubmit = layer.getChildFrame('#form_su', index);
                            // 获取表单中的提交按钮（在我的表单里第一个button按钮就是提交按钮，使用find方法寻找即可）
                            // formSubmit.submit();
                            //var submited = formSubmit.find('.form-builder-submit').find('button')[0];
                            // 触发点击事件，会对表单进行验证，验证成功则提交表单，失败则返回错误信息
